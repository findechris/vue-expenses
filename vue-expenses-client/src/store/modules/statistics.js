...
import {
    SET_CATEGORIES_BREAKDOWN,
    SET_EXPENSES_BREAKDOWN,
    UPDATE_STATISTICS,
    ADD_NEWCATEGORY_STATISTICS,
    UPDATE_CATEGORY_STATISTICS,
    REMOVE_CATEGORY_STATISTICS
...
const actions = {
    [LOAD_CATEGORIES_BREAKDOWN]({ commit }) {
        return Api.get('/statistics/getcurrentyearcategoriesbreakdown').then(
            (response) => {
    ...
    [LOAD_EXPENSES_BREAKDOWN]({ commit }) {
        return Api.get(
            '/statistics/getcurrentyearexpensesbycategorybreakdown'
        ).then((response) => {
...
const mutations = {
    ...
    [UPDATE_STATISTICS](state, payload) {
        ...
        if (expenseMonth == currentmonth) {
            ...
            var currentmonthData = state.categorybreakdown.filter((o) => {
                return o.month == currentmonth
            })
            var category = currentmonthData.filter((o) => {
                return o.name == payload.expense.category
            })
            if (category[0]) {
        ...
        var expensebreakdown = state.expensesbreakdown.filter((o) => {
            return (
                o.month == expenseMonth &&
                o.categoryName == payload.expense.category
            )
        })
        if (expensebreakdown[0]) {
            ...
            expenseDate.getYear() == new Date().getYear() &&
    ...
    [UPDATE_CATEGORY_STATISTICS](state, category) {
        var categories = state.categorybreakdown.filter((o) => {
        ...
        var expenses = state.expensesbreakdown.filter((o) => {
            return o.id == category.id
        })
    ...
    [REMOVE_CATEGORY_STATISTICS](state, categoryId) {
        state.categorybreakdown = state.categorybreakdown.filter((o) => {
            return o.id != categoryId
        })
        state.expensesbreakdown = state.expensesbreakdown.filter((o) => {
            return o.id != categoryId
        })
    }
}
...
const getters = {
    monthlyBudget: (state, getters, rootState) => {
        var currentmonth = new Date().getMonth() + 1
        var currentMonthBudget = state.categorybreakdown.filter((o) => {
            return o.month == currentmonth
        })
        var totalBudget = sumBy(
            rootState.expenseCategories.categories,
            (ec) => {
                return ec.budget
            }
        )
        var totalSpent = sumBy(currentMonthBudget, (cm) => {
            return cm.spent
        })
            ...
            data: [
                {
                    ...
                    name: 'Spent',
                    itemStyle: { color: '#2779bd' }
                },
                {
                    value: (remaining < 0 ? 0 : remaining).toFixed(2),
                    name: 'Remaining',
                    itemStyle: { color: '#BDBDBD' }
                }
            ],
            totalBudget: new Intl.NumberFormat(
                window.navigator.language
            ).format(totalBudget.toFixed(2)),
            totalSpent: new Intl.NumberFormat(window.navigator.language).format(
    ...
    monthlyBudgetsByCategory: (state) => {
        var currentmonth = new Date().getMonth() + 1
        var currentMonthBudgets = state.categorybreakdown.filter((o) => {
            return o.month == currentmonth
        })
        var groupedBugetsByCategory = groupBy(currentMonthBudgets, (e) => {
            return e.name + '|' + e.colour
        })
        return map(groupedBugetsByCategory, function (budget, key) {
            ...
            return {
                ...
                monthlyBudget: [
                    {
                        value: sumBy(budget, 'spent').toFixed(2),
                        name: 'Spent',
    ...
    yearlyExpenses: (state) => {
        ...
        var groupedByMonths = groupBy(state.expensesbreakdown, (e) => {
            return e.month
        })
        var yearlyExpenses = {
            xAxisData: [],
    ...
    categoryExpenses: (state) => {
        ...
        var groupedByCategory = groupBy(state.expensesbreakdown, (e) => {
            return e.categoryName + '|' + e.categoryColour
        })
