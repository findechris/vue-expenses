...
const actions = {
    [LOAD_CATEGORIES]({ commit }) {
        Api.get('/expensecategories').then((response) => {
    ...
    [CREATE_CATEGORY]({ commit, dispatch }, { expenseCategory }) {
        return Api.post('/expensecategories', {
            ...
            dispatch(
                `statistics/${CREATE_NEWCATEGORY_STATISTICS}`,
    ...
    [EDIT_CATEGORY]({ commit, dispatch }, { expenseCategory }) {
        return Api.put(
            ...
            dispatch(
                `alert/${ADD_ALERT}`,
                {
                    message: 'Expense category updaded successfully',
                    color: 'success'
                },
                { root: true }
            )
            dispatch(
                `statistics/${EDIT_CATEGORY_STATISTICS}`,
                { category: expenseCategory },
                { root: true }
            )
        })
    ...
    [REMOVE_CATEGORY]({ commit, dispatch }, { id }) {
        Api.delete(`/expensecategories/${id}`).then(() => {
            ...
            dispatch(
                `expenses/${REMOVE_EXPENSESOFCATEGORY}`,
                { categoryId: id },
                { root: true }
            )
            dispatch(
                `statistics/${LOAD_CATEGORIES_BREAKDOWN}`,
                {},
                { root: true }
            )
            dispatch(
                `statistics/${LOAD_EXPENSES_BREAKDOWN}`,
                {},
                { root: true }
            )
        })
...
const mutations = {
    ...
    [UPDATE_CATEGORY](state, expenseCategory) {
        let expenseCategoryUpdated = state.categories.find(
