...
const actions = {
    [LOAD_EXPENSE_TYPES]({ commit }) {
        Api.get('/expensetypes').then((response) => {
    ...
    [EDIT_EXPENSE_TYPE]({ commit, dispatch }, { expenseType }) {
        return Api.put(`/expensetypes/${expenseType.id}`, expenseType).then(
            (response) => {
                let expenseType = response.data
                ...
                dispatch(
                    `alert/${ADD_ALERT}`,
                    {
                        message: 'Expense type updaded successfully',
                        color: 'success'
                    },
                    { root: true }
                )
    ...
    [REMOVE_EXPENSE_TYPE]({ commit, dispatch }, { id }) {
        return Api.delete(`/expensetypes/${id}`).then(() => {
            ...
            dispatch(
                `expenses/${REMOVE_EXPENSESOFTYPE}`,
                { typeId: id },
                { root: true }
            )
            dispatch(
...
const mutations = {
    ...
    [UPDATE_EXPENSE_TYPE](state, expenseType) {
        let expenseTypeUpdated = state.types.find(
            (et) => et.id == expenseType.id
        )
